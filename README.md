...
Based on the data analysis, city types with less drivers have fewer rides and higher fares. This can be seen by the disparity between rural and urban city types. The next steps would be to determine if less drivers directly causes fewer rides, or if higher fare prices results in less rides. Lower number of drivers could result in less rides due to wait times and overall availabliliy. In addition, higher fare prices may deter people from choosing to use the app. To further analyze, I would recommend creating scatter plots. Scatter plots can visually depict where there is a relationship between two factors. Instead of indexing based on "Date", an additional DataFrame could be created with an index of fare price and evaluate ride count values. Also, create a DataFrame with a driver count index and evaluate ride count values. Once, scatter plots for ride count versus fare price and driver count, a regression analysis can be performed to determine the correlation. Finally, I would suggest that if driver availability and affordability are the company's goal, then the driver count for rural areas needs to be increased and the company should focus on ways to make the app affordable for all city types. 
